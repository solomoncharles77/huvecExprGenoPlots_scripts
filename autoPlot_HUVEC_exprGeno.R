...
expr <- data.frame(fread("/scratch/cellfunc/shared/HUVEC_BackUp_Oct22/HUVEC_RNAseq/Counts/salmon.merged.gene_tpm.tsv"))
...
candexpr <- expr[expr$ensembl_gene_id %in% cand, ]
candName <- candexpr$external_gene_name
...
ggsave(paste0(cand, "_vs_", rs, "_Plot.png" ), dfPlot,
       scale = 1,
       path = paste0("exprGenoPlot/huveExprGenoPlot"),
       width = 80,
       height = 70,
       units = "mm",
       dpi = 300,
